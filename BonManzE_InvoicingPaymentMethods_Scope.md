...
```
**Revised after Antigravity's technical review (2026-09-10) — the `SYSTEM_CONFIG`-level fallback originally proposed here is dropped.** The original draft proposed matching `defaultInvoicePrefix`/`defaultInvoiceNumberCounter` fields on `SYSTEM_CONFIG` for orders with no assigned entity. Reading the actual `confirmCheckout` function directly (`functions/index.js`) during review shows this path can't happen: `confirmCheckout` already hard-fails before any order is created if `!customer.entityId` (`"Your account has not been assigned to a trading entity."`) — every order that reaches the point where an invoice number would be generated already has a real, validated entity. The only orders without an `entityId` are ones from before the Multi-Entity system existed at all, which already fall under §6's no-retroactive-numbering rule (raw order ID fallback) regardless. So the system-level fallback fields are dead code before they'd ever be written — removed from scope rather than built and left unreachable.

**Counter mechanics:** `invoiceNumberCounter` is a single running integer on the entity document (no year-keying, no map — one number). `confirmCheckout` must read it via `tx.get(entityRef)` **inside** the same Firestore transaction that writes the order — not the `entitySnap` the function already reads earlier (in the initial parallel `Promise.all`, before the transaction starts) for pricing/display purposes. That earlier read is fine for display fields, which don't need transactional freshness, but reusing it for the counter would defeat the entire point: two checkouts against the same entity landing in the same moment could both read the same stale count and both compute the same "next" number. Confirmed this matches what Antigravity's own review already proposed for this piece — flagged here explicitly so it's locked in as a requirement, not left as an implementation detail someone could get wrong under time pressure. This is the same reason `total`/`subtotal`/`vat` are server-computed rather than client-written today.
...
5. **Invoice/order numbering:** the entity's `invoicePrefix`/`invoiceNumberCounter` fields (no `SYSTEM_CONFIG`-level fallback needed — see §5's revision), the transactional per-entity counter (read via `tx.get()` inside the same transaction that writes the order), `confirmCheckout`'s composed-and-frozen `invoiceNumber` (prefix + continuous 9-digit sequence, no reset), and the receipt's ref display switching over (with the no-retroactive-numbering fallback for older, pre-entity orders).
