...
## 2026-09-15 — Payment Grouping, Bulk Delivery Actions, Receipt Unification & Customer/Admin Invoice Parity (Claude, committed by Antigravity)

**Commits:** [`519f937`](https://github.com/BhimalL/BonManze_PWA/commit/519f937950578cc71802aabe01ad41e0a6845cb0) "feat(receipt): extract shared ReceiptModal, persist standardLabel, fix customer view reprint count bump", [`73e762a`](https://github.com/BhimalL/BonManze_PWA/commit/73e762a2f5a2b6ae0222841288e74aefa1e56376) "feat(payments): per-entity payment reference counter and mintPaymentReference callable function", [`afb94cf`](https://github.com/BhimalL/BonManze_PWA/commit/afb94cf7da7f5d607e4c01eb8d0a8453a1c26992) "feat(operations): group pending payment drops by reference code and batch mark paid", [`b2624e0`](https://github.com/BhimalL/BonManze_PWA/commit/b2624e01f76416c48797394b0620ad334e1a3324) "feat(delivery): bulk dispatch and mark delivered actions for Delivery List tab", and [`e263863`](https://github.com/BhimalL/BonManze_PWA/commit/e263863d59f2dfe6391a56fc4bb16590122952e2) "fix(customer-portal): group receipts by drop key for 1-to-1 invoice parity" — all pushed to `main`.

**Verified:** all five commits independently checked by Claude via `.git/logs/HEAD` (genuine `pull origin main: Fast-forward` entries, unbroken linear parent chain, no rebasing or force-push) plus direct source diff against the real files on Bhimal's machine before being trusted, per the Working Agreement. `npx tsc --noEmit`, `node --check functions/index.js`, and `npx vite build` all ran clean, both after each individual commit and again together in a full combined check across all five changed files at the end of this session. **Confirmed working:** Bhimal's live use of the Payments and Delivery List tabs confirmed the per-entity payment reference counter (`73e762a`), payment-reference grouping with a consolidated "Mark All Paid" action (`afb94cf`), and bulk Dispatch/Mark Delivered checkbox actions (`b2624e0`) all behave as intended. The receipt-unification work (`519f937`) and the customer/admin invoice-parity fix (`e263863`) are Verified only — correct by source review and clean build, but not yet exercised live by Bhimal.

**Changes:**
- **Shared receipt modal & reprint-count fix (`519f937`)** — Extracted a single `ReceiptModal` used by both admin and customer views, persisted `standardLabel` onto the printed receipt, and fixed a bug where reopening a receipt from the customer view was incorrectly bumping its reprint counter.
- **Per-entity payment reference counter (`73e762a`)** — Added a `mintPaymentReference` callable Cloud Function that mints a sequential, per-entity payment reference using each entity's configured `paymentRefPrefix`, falling back to a random `BMZ-PAY-`+6-digit reference when an entity has no prefix configured or no entity is set at all.
- **Payment-reference grouping in Operations (`afb94cf`)** — Pending payment drops in the admin Payments tab that share a claimed payment reference across more than one drop are now grouped into a single card with one consolidated "Mark All Paid (N)" action — one `writeBatch` and one audit-log entry across every drop/order in the group, instead of marking each drop paid individually. A solo drop (no shared reference) still gets its own card exactly as before.
- **Bulk Dispatch / Mark Delivered in Delivery List (`b2624e0`)** — Added checkbox-based multi-select (per drop, plus a per-section "Select all") to the Delivery List tab, with a sticky bulk-action toolbar. Bulk Dispatch only acts on items still `Ready`; bulk Mark Delivered only acts on items already `En route` — that restriction was an explicit design decision from Bhimal, stricter than the existing single-drop Mark Delivered action.
- **Customer/admin invoice parity fix (`e263863`)** — Root cause of the inconsistency Bhimal caught (admin's Transactions Ledger correctly printed two separate invoices for a single grouped payment, while the customer app showed one combined receipt for that same payment) was that `CustomerPortal.tsx` grouped receipts by `paymentReference`, while the admin side has always grouped by drop (order + delivery date + service slot). Changed the customer portal's grouping key to match the admin side's drop key exactly, so the customer now sees the same two separate invoices admin does, even though both were paid together in one grouped payment.

**Design note carried over from this session's discussion, unchanged by any of the above:** invoice numbering stays deliberately decoupled from however a payment happened to be bundled — two dishes delivered in the exact same slot (same order, date, and service slot) are one drop and share one invoice number, but two different drops paid together via one grouped payment reference still get two separate invoice numbers, one per drop, exactly as before this session's changes. Confirmed with Bhimal as intended behavior, not a bug, per `BonManzE_InvoicingPaymentMethods_Scope.md` §8 — the payment reference is a reconciliation aid only and never determines invoice count.

**Flagged, not blocking — payment-reference collision risk:** the random fallback reference minted by `73e762a` (`BMZ-PAY-` + 6 random digits, used only when an entity has no `paymentRefPrefix` configured or no entity is set) is not scoped per entity or per customer. Because `afb94cf`'s grouping now merges any pending drops that share a reference into one card, two unrelated customers who happened to collide on the same random 6-digit reference (roughly a 1-in-900,000 chance per pair) would have their unrelated drops incorrectly grouped together in the Payments tab. This risk already existed at a smaller scale once the reference feature shipped; grouping makes a collision slightly more consequential, though still a display/grouping mix-up rather than a data-integrity issue — each drop still belongs to its own order underneath, and a mis-grouped card would look obviously wrong (mismatched customer names) if it ever happened. **Closed 2026-09-15 — see below.**

---

## 2026-09-15 (2) — Mandatory Entity Prefixes & Reprint-Badge Scoping (Claude, committed by Antigravity)

**Commits:** [`63771f2`](https://github.com/BhimalL/BonManze_PWA/commit/63771f2dd2ea9e139f02447aa4148a297faf069e) "feat(entities): require invoice and payment ref prefixes on trading entities" and [`3b9a5d9`](https://github.com/BhimalL/BonManze_PWA/commit/3b9a5d979c387c9ee9c0897b294a75369e960acf) "fix(customer-portal): never show Duplicate/Reprint badge to customers" — both pushed to `main`.

**Verified:** both commits independently checked by Claude via `.git/logs/HEAD` (genuine `pull origin main: Fast-forward` entries, parent hash matching the previous commit exactly) plus a direct source diff against the real files on Bhimal's machine before being trusted, per the Working Agreement. `npx tsc --noEmit` and `npx vite build` ran clean for both. **Confirmed working:** Bhimal live-tested the mandatory-prefix validation in the Trading Entity form (leaving Payment Ref Prefix blank correctly blocked Save with the error message) and confirmed the receipt-unification work from the prior entry (`519f937`) renders identically between admin and customer views.

**Changes:**
- **Mandatory entity prefixes (`63771f2`)** — Directly closes the payment-reference collision risk flagged in the entry above. Invoice Prefix and Payment Ref Prefix are now required fields (red asterisk, inline validation) on the Trading Entity Add/Edit form in Settings, on both create and edit — an existing entity can no longer have its prefix cleared out either. Both of Bhimal's existing entities (BonMazE, Rik's Kitchen) already had both fields set, so no cleanup was needed on existing data.
- **Reprint badge scoped to admin only (`3b9a5d9`)** — Found while live-testing the reprint-count fix from the prior entry (`519f937`): the admin Transactions Ledger correctly showed the "Duplicate / Reprint" badge on a genuine re-print, but the same badge was also appearing in the *customer* app on receipts the customer was opening for the first time — because `invoiceReprintCount` is one shared counter, bumped only by admin/staff prints, and the customer portal's badge was reading that same field. A customer had no way to know staff had printed a copy for their own records, so seeing "Duplicate" on what was, from their side, a first view read as a bug. Fixed by hard-coding the customer portal's `anyReprinted` to `false` — reprint tracking is now admin-only end-to-end, not just on the write side as `519f937` left it.

**Net result:** this closes out every open item from the 2026-09-15 (1) entry above — the collision risk is fixed at the source, and both previously-unverified features (receipt unification, invoice parity) are now confirmed working live, alongside catching and fixing one new bug (the reprint badge leak) that only surfaced during that live testing.

---

## 2026-09-16 — Delivery Payments: Rules Expression Cap & Payment Button Permission Gating (Claude, committed by Antigravity)

**Commit:** [`a33daab`](https://github.com/BhimalL/BonManze_PWA/commit/a33daabbc5101ac2c8905a0661d559cff345e2ee) "fix(delivery-payments): resolve rules expression cap and payment-button permission gating" — pushed to `main`.

**Verified:** independently checked by Claude via `.git/logs/HEAD` (genuine `pull origin main: Fast-forward` from `3d620f88` to `a33daab`, `refs/heads/main` and `refs/remotes/origin/main` both matching), tree diff confirming only `firestore.rules`, `modules/Operations.tsx`, and `AGENTS.md` changed, and both edited files' SHA1 hashes matching exactly what was delivered before handoff. `npx tsc --noEmit` clean. **Confirmed working:** Bhimal live-tested both fixes on the Test_Driver account — the driver's Deliver & Collect write (previously blocked by the 1000-expression cap) now completes and shows correctly as a driver claim in the Payments tab; Mark Paid/Send back buttons now render dimmed/inert on an account without Payments → Edit, matching the role's actual permissions.

**Changes:**
- **Firestore rules expression cap fix** — the Delivery Staff Payments feature (`3d620f88`) added a 4th OR clause to the `items/{itemId}` `allow update` rule on top of three pre-existing ones. Each clause called `isStaffAllowed(...)`-family functions that re-fetched `roles/{roleId}` via `get()` up to four times per call, and `isCustomer(...)`/`isStaffAllowed('deliveryList','edit')` were each re-evaluated redundantly across clauses — pushing the total per-write expression count over Firestore's hard 1000-expression cap, surfaced as `PERMISSION_DENIED: ... maximum of 1000 expressions ...` the first time a driver used Deliver & Collect. Fixed by memoizing the role-document lookup (down to one `get()` per `isStaffAllowed` call, via a `let` binding) and computing the two genuinely-repeated checks once instead of inline per clause — a pure performance refactor with no change to what is allowed or denied (independently re-verified clause-by-clause for every account type before shipping).
- **Payment button permission gating fix** — Mark Paid, Mark All Paid, and Send back in the Payments tab were visible and clickable for any staff member regardless of their actual `payments.edit` permission; only the payment-method-picker buttons inside the Mark Paid modal enforced it. No unauthorized write was ever actually possible (both `markPaidGroup`/`resetPaymentClaim` and `firestore.rules` already required `payments.edit` server-side), but the inconsistency was misleading — caught by Bhimal reviewing the Payments tab on a driver account. Fixed by adding the same permission check to all four buttons (with an explanatory tooltip when disabled), confirmed with Bhimal as the intended access model: controlled purely through the role's Payments View/Edit checkboxes, with no separate hardcoded rule for driver accounts.

**Follow-on scoped, not yet built:** a cashier "Reconcile" section (pick a driver, confirm their pending claims individually or in bulk by payment method) plus a driver-facing "Claimed" dashboard card and hand-over breakdown — see `BonManzE_DeliveryPayments_Scope.md` §9 in the Claude Project for the full design, decided directly with Bhimal 2026-09-16.

---

## 2026-09-17 — Delivery & Payments: Expression-Cap Regression Fix (Round 2) & RBAC/Apportionment Test Coverage (Claude, committed by Antigravity)

**Commits:** [`77d2f94`](https://github.com/BhimalL/BonManze_PWA/commit/77d2f9425daee2fe81dab7b3cc5e98a935a450cd) "fix(delivery-payments): resolve recurring rules expression-cap regression, add RBAC + apportionment test coverage" and [`5245f58`](https://github.com/BhimalL/BonManze_PWA/commit/5245f5888d303bd06d6a2c1d8b90db4002321b29) "docs: log delivery-payments rules expression-cap fix & test suites in AGENTS.md" — both pushed to `main`.

**Verified:** independently checked by Claude via `git fetch origin` (local `HEAD` matching `origin/main` exactly at `5245f58`, unbroken parent chain, no rebase/force-push), a full source diff of `firestore.rules` against `77d2f94` (exactly the `canUpdateItem()` rewrite described below, 36 insertions / 5 deletions, no unrelated changes), and an md5 checksum match between the committed `firestore.rules` and the copy that had already passed the RBAC suite before handoff. `npx tsc --noEmit` clean. **Confirmed working:** `scripts/testDeliveryPaymentsRBAC.js` — 17/17 assertions passing against a live Firestore emulator, verified to be genuine `PERMISSION_DENIED`/`false` denials rather than cap-triggered false positives (see root cause below); `scripts/testApportionmentMath.js` — 16/16 passing.

**Changes:**
- **Expression-cap regression, round 2** — the `canUpdateItem()` memoization shipped in `a33daab` (2026-09-16, logged above) fixed the *reported* cap failure but didn't go far enough: every distinct permission check (`payments.edit`, `payments.view`, `ordersByDish.edit`, driver-claim) was still independently re-deriving the whole staff/role lookup chain from scratch. That's cheap for a write that matches an early OR-clause (short-circuits), but a write matching *no* clause — exactly what a "correctly rejected" RBAC test exercises — forces full evaluation of every clause, which could still exceed Firestore's hard 1000-expression cap and surface as `PERMISSION_DENIED: ... maximum of 1000 expressions ...` instead of a clean `false`. Because both a cap-triggered denial and a genuine logic-based denial return the same `permission-denied` error code, a test suite that only checks the error code (not the full error text) could report a false-positive pass. Fixed by rewriting `canUpdateItem()` to fetch the staff doc, role doc, and permissions map exactly once via `let` bindings, then deriving every permission flag (`canEditPayments`, `canViewPayments`, `canEditOrdersByDish`, `canEditDeliveryList`, `isDriver`) from that single lookup instead of re-calling `isStaffAllowed(...)`/`isDriverStaff()` inline per clause.
- **New RBAC test suite (`scripts/testDeliveryPaymentsRBAC.js`, 370 lines)** — 17 assertions against a live Firestore emulator covering every `items/{itemId}` update path (payments edit/view, orders-by-dish edit, delivery-list edit, driver claim, and the outer `isPartnerEntityAllowed`/`isDriverItemAllowed` gates) for both allowed and denied cases, reading the full error detail (not just the error code) so a cap-triggered denial can never masquerade as a correct one again.
- **New apportionment test suite (`scripts/testApportionmentMath.js`, 102 lines)** — table-driven unit tests (16 cases) for the net-price proportional apportionment math used by Deliver & Collect, covering 2-item even split, 3-item repeating-decimal split, single item, and multi-drop group scenarios.
- **`utils/apportionment.js` (new, 47 lines)** — standalone extraction of the apportionment algorithm to make it unit-testable in isolation. **Not yet wired into `modules/Operations.tsx`** — the production apportionment logic remains inline there; this file exercises the algorithm in parallel for test coverage only. Flagged as a follow-up refactor opportunity (have `Operations.tsx` import this shared implementation instead of keeping two copies), not done in this round to keep the fix low-risk.
- **`AGENTS.md` (`5245f58`, unprompted by Antigravity)** — logged the above in the project's own agent-coordination doc. **Correction needed:** that entry describes the `paymentPaidAmount`/`paymentWrittenOff` field-pinning as using `affectedKeys().hasAny(...)` checks; the actual shipped code (confirmed against the real `firestore.rules`) uses `request.resource.data.get(field, null) == resource.data.get(field, null)` pin comparisons instead — the `affectedKeys()` idiom was from an earlier draft that was reviewed and reverted before this round's commit. See the correction applied directly to `AGENTS.md` alongside this entry.

**Root-cause note, carried over for future reference:** a transient compile error (`Unexpected '<EOF>'`) seen twice during this round's live testing was traced to the Firestore Emulator's own background file-watcher racing a multi-step file write (catching a torn/incomplete snapshot mid-edit), not a real syntax defect — confirmed via a comment/string-aware bracket-balance check showing the file was always well-formed, and directly observed in the emulator's own log ("Change detected, updating rules..." → the error → "Rules updated." within one contiguous excerpt, self-resolving on the next watch cycle). Worth remembering if this recurs: check whether the file settles and reloads cleanly on its own before assuming the content is broken.

**Flagged, not blocking:**
- The pre-existing `allow update` line's own `isPartnerEntityAllowed()`/`isDriverItemAllowed()` calls still independently re-derive `isActiveStaff()`/`staffDoc()` redundantly with `canUpdateItem()`'s internal fetch — a further, smaller source of near-the-cap risk. Deliberately left untouched this round to keep the fix scoped; revisit if another clause is ever added to this rule.
- `scripts/apportionment.js` (a byte-identical duplicate of `utils/apportionment.js`) and `scripts/diagnose_reload.js` (a scratch diagnostic used while investigating the transient compile error above) were excluded from this commit as unused/debug-only.
