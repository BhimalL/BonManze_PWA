...
**Still open — real blockers before any code gets written, not busywork:**
- **The two entities' real legal names, BRN/VAT registration numbers, bank/payment references, and logo/branding assets.** Nothing can seed an `entities` collection without this — pure business/legal information only Bhimal has.

**Not started:** no Firestore schema, security rules, or Cloud Function work for this feature yet. `BonManzE_MultiEntity_ImplementationPlan.md` (new, 2026-08-25) is the technical implementation plan reflecting all five settled answers above, written for Antigravity's review before any of it is built — mirrored into the repo (`BonManzE_MultiEntity_ImplementationPlan.md` at repo root) alongside a synced, up-to-date copy of this scope document, since the repo's own copy of this file had gone stale since 2026-08-10.
