...
- **Pay Sheet Payment Methods Gating (`modules/CustomerPortal.tsx`)**: Added `entities` real-time listener to `CustomerPortal.tsx`. Updated `currentPayEntityId` and `applicablePaymentMethods` filtering to resolve entity ID across all pay target kinds (`item`, `order`, `balance`, `day` via `payTarget.items[0].orderId`) with single-entity fallback (`entities[0]`), strictly restricting visible payment options in the Pay Sheet drawer to the entity's `acceptedPaymentMethodIds`.
- **Operations Collect Payment Gating (`modules/Operations.tsx`)**: Updated `Collect Payment` modal options to filter against the order entity's `acceptedPaymentMethodIds` with single-entity fallback (restricting options to Juice & Cash for Rik's Kitchen).
...
- **Discount Line Display Split (`modules/CustomerPortal.tsx`)**: Split order totals footer discount line into three explicit rows (Standard/Group, 🎂 Birthday, Full-Week) matching pre-checkout preview style.
- **Account-Scoped Cart Storage (`modules/CustomerPortal.tsx`)**: Scoped draft cart `localStorage` keys to `currentUser.id` (`bmz_customer_cart_lunch_<id>`) and added `useEffect` listener to prevent draft cart leaking across customer account switches.
- **Per-Item Discount Attribution (`functions/index.js`, `types.ts`, `CustomerPortal.tsx`, `Operations.tsx`)**: Added `discountShare: { standard, birthday, bulk }` to `OrderItem` written by `confirmCheckout` at checkout. Updated item mapping whitelists in `CustomerPortal.tsx` (line ~866) and `Operations.tsx` (line ~1044) to include `discountShare`. Updated Pay Day/Balance/Item amounts and pro-rated receipts across Customer Portal and Operations Console to use exact item discount shares rather than order-wide blended estimates (`Claude outputs/discount-attribution-and-session-summary.md`).
- **Currency Decimal Formatting Cleanup (`modules/CustomerPortal.tsx` & `modules/Operations.tsx`)**: Standardized 16 raw `Rs {value}` string interpolations to use `formatCurrency(value)` (e.g. `Rs 227.01` instead of `Rs 227.01000000000002`) across both customer portal and operations console (`Claude outputs/currency-decimal-formatting-cleanup.md`).
...
- **Tracked Test & Verification Helpers (`scripts/resetTestData.js`, `scripts/verifyDiscountShare.js`, `scripts/checkInvoiceNumbering.js`)**: Tracked Firestore reset script `scripts/resetTestData.js`, read-only discount attribution verification script `scripts/verifyDiscountShare.js`, and invoice numbering diagnostic script `scripts/checkInvoiceNumbering.js` directly in git repository to preserve test utilities across `git clean -fd` resets. Updated `resetTestData.js` to target `auditLog` (singular) collection matching production schema.
- **Shared Receipt Component & Standard Discount Label Persistence (`modules/Receipt.tsx`, `types.ts`, `functions/index.js`, `CustomerPortal.tsx`, `Operations.tsx`)**: Extracted unified `ReceiptModal` component into `modules/Receipt.tsx`. Added `standardLabel` to `discountBreakdown` schema in `types.ts` and `confirmCheckout` server function in `functions/index.js`. Fixed `CustomerPortal.tsx` to stop incrementing `invoiceReprintCount` on receipt view, reserving reprint counts strictly for explicit print actions in `Operations.tsx`.
- **Per-Entity Sequential Payment Reference Counters (`types.ts`, `functions/index.js`, `Operations.tsx`, `CustomerPortal.tsx`)**: Added `paymentRefPrefix` and `paymentRefCounter` to `Entity` schema in `types.ts` and Trading Entity settings form in `Operations.tsx`. Implemented `mintPaymentReference` Cloud Function in `functions/index.js` to transactionally generate sequential payment reference codes (e.g., `Pay-Rks-000000005`) per entity, with fallback to legacy `BMZ-PAY-XXXXXX` when unconfigured. Wired Customer Portal Pay Sheet in `CustomerPortal.tsx` to mint references on payment method selection with race guard and loading state.
- **Payment Drops Reference Grouping (`modules/Operations.tsx`)**: Merged pending payment drops sharing a claimed payment reference into a unified `PaymentGroup` card in Operations Console with date-range badges, individual print/send-back actions per constituent drop, and a consolidated `Mark All Paid (N)` action executing a single `writeBatch` write and audit log entry.
- **Bulk Delivery List Actions (`modules/Operations.tsx`)**: Added multi-drop checkboxes, section-level select-all toggles, and a sticky bulk action toolbar to the Delivery List tab in Operations Console, enabling one-click bulk `Dispatch (N)` for `Ready` items and strict bulk `Mark Delivered (N)` for `En route` items with single `writeBatch` writes and consolidated audit log entries.
- **Customer Receipt Drop Grouping (`modules/CustomerPortal.tsx`)**: Updated `paymentGroups` and `openReceipt` in Customer Portal to group receipts by drop (`orderId + deliveryDate + serviceSlot`) matching backend invoice assignments. Removed order-header "Paid · Receipt" combined button in order cards so each receipt corresponds 1-to-1 with a single issued invoice.
- **Mandatory Entity Prefixes Validation (`modules/Operations.tsx`)**: Made Invoice Prefix and Payment Ref Prefix required fields on the Trading Entity Add/Edit modal, preventing form submission if either field is missing.
- **Customer Receipt Reprint Badge Visibility (`modules/CustomerPortal.tsx`)**: Hardcoded `anyReprinted: false` in Customer Portal receipt data so customers never see "Duplicate / Reprint" badges (reserving reprint tracking exclusively for Operations Console prints).
- **Delivery Staff Accounts & Cash Reconciliation (`types.ts`, `firestore.rules`, `functions/index.js`, `modules/Operations.tsx`)**: Added `isDriver` flag to Staff schema and Roles & Staff UI, driver picker at Dispatch, driver-scoped Delivery List and Payments views, driver Deliver & Collect modal (with Paid/Partial/Issue claim recording), driver claim attribution in Payments tab, and daily per-driver cash reconciliation panel in Operations Console.
- **Security Rules Expression Cap & Payment Button Permission Gating (`firestore.rules`, `modules/Operations.tsx`)**: Memoized `isStaffAllowed`'s `roles/{roleId}` lookup in `firestore.rules` (reducing expression count below 1000 limit during driver delivery writes). Updated `Mark Paid`, `Mark All Paid`, and `Send back` buttons in `Operations.tsx` to disable with explanatory tooltips when staff lack `payments.edit` permission.
- **Cashier Reconcile Section & Driver Claimed Overhaul (`modules/Operations.tsx`)**: Added cashier Reconcile section to Payments tab for confirming driver claims individually or via atomic `markPaidBulk`, replaced driver's "Collected" card with a driver-specific "Claimed" card, added "Driver Claimed" summary card for back office, and added date/service scoping filter bar across Payments tab.
- **Amount-aware Resolve/Write Off & Apportioned Deliver & Collect (`types.ts`, `firestore.rules`, `modules/Operations.tsx`, `modules/CustomerPortal.tsx`, `modules/Receipt.tsx`)**: Added `paymentPaidAmount` & `paymentWrittenOff` fields to `OrderItem` schema and `firestore.rules`. Apportioned Deliver & Collect collected amounts across multi-item drops by net-price share. Added `writeOffClaim` flow and updated Customer Portal / Receipts to subtract paid amounts from outstanding balances.
...
## 2026-08-25: Claude — Multi-Tier Trading Entity System, placeholder entity data added and production grandfathering formally decided

Two remaining loose ends on this feature closed with Bhimal — both now reflected in `BonManzE_MultiEntity_ImplementationPlan.md` and `BonManzE_v1_scope.md` (re-synced to the repo alongside this entry):

1. **Entity seed data.** The two entities' real legal name/BRN/VAT number/bank reference/logo were still pending real business info from Bhimal. Rather than block build work on that, Bhimal provided placeholder values — deliberately obviously-fake (e.g. `PLACEHOLDER-BRN-A`, not a real-looking BRN format) so nobody mistakes them for genuine legal data. See the plan's new §3 for the full table (both entities, all fields). These go into the two `entities/{entityId}` seed documents at build time (step 1 of the build sequence, §10) and get swapped for the real values as a go-live checklist item — a plain data update, not a schema or code change.
