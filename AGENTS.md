...
- **Operations Collect Payment Gating (`modules/Operations.tsx`)**: Updated `Collect Payment` modal options to filter against the order entity's `acceptedPaymentMethodIds` (restricting options to Juice, MauCAS, Cash for Rik's Kitchen).
